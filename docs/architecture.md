...
DeepSeek 适配与 AI HTTP 路由属于 API 边界，而不是 `packages/core`。这能保证核心包在离线环境、测试和 CLI 中没有隐式网络行为。

## 4. 核心数据流

### 4.1 输入

基线与候选输入可以是 YAML 文本、JSON 文本或已解析对象。解析阶段首先检查顶层结构和 `openapi` 版本字段，并对无法解析的内容返回结构化错误。文件扩展名不是可信依据。
...
AI 调用与创建分析是两个独立动作。只有客户端对一个已存在的分析显式调用 `POST /api/analyses/{id}/ai-review` 时，API 才会访问 DeepSeek：
...
sequenceDiagram
    ...
    U->>A: POST /api/analyses/{id}/ai-review
    A->>A: Check enabled, config, language and focus
...
- `DEEPSEEK_API_KEY` 只存在于服务端环境，浏览器请求和响应均不包含它；
...
- 发送给 DeepSeek 的 finding 可能含内部端点名称，启用前必须做数据分级、脱敏和云服务合规评估；
...
| DeepSeek 超时 | AI 路由返回 504；保留原分析并允许稍后重试 |
| DeepSeek 拒绝或服务异常 | AI 路由返回 502；不把上游正文或 key 泄露给客户端 |
...
API 兼容判定需要可复现和可审计，因此规则引擎是严重等级、得分与 CI 的唯一来源。DeepSeek 只对已经产生的 finding 做自然语言解释，并通过独立、显式调用触发。这样既能提供迁移叙事，也不会让随机输出进入发布门禁。
